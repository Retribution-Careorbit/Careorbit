...
-   **OrbitScoreBadge:** Compact 36px circular badge in the header bar (always visible, even when sidebar is closed). Shows score number with a ring progress indicator, color-coded (rose/amber/cyan/emerald). Clicking navigates to `/orbit-score`. Also, CareOrbit logo is displayed in the header bar for persistent branding.
